		...
		*/
		...
		var app	={
			...
			getYoutubeVideoId: function(url){
				if(!url) return '';
				let match = url.match(/(?:youtube\.com\/watch\?v=|youtube\.com\/embed\/|youtu\.be\/)([^&?/]+)/i);
				return match ? match[1] : '';
			},
			buildYoutubeEmbedUrl: function(){
				let youtube = app.db.youtube || {};
				let videoId = app.getYoutubeVideoId(youtube.url || '');
				if(!videoId) return '';
				let mute = app.isEnabled(youtube.mute) ? 1 : 0;

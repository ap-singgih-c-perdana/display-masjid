...
var app = {
	...
	db		: null,
	testAudio	: null,
	processUrl: (window.ADMIN_CONFIG && window.ADMIN_CONFIG.processUrl) ? window.ADMIN_CONFIG.processUrl : "proses.php",
	displayBaseUrl: (window.ADMIN_CONFIG && window.ADMIN_CONFIG.displayBaseUrl) ? window.ADMIN_CONFIG.displayBaseUrl : "/display",
	...
	testBeep: function(self){
		...
		if(!app.testAudio){
			app.testAudio = new Audio(app.displayBaseUrl + '/img/beep.mp3');
		}
    ...
    initialize: function() {
		...
		$(document).on('click', '.js-refresh-active-menu', function(event){
			event.preventDefault();
			$('.sidebar-menu .active a').trigger("click");
		});

		$(document).on('click', '.js-update-clock', function(event){
			event.preventDefault();
			app.updateClock(this);
		});

		$(document).on('click', '.js-test-beep', function(event){
			event.preventDefault();
			app.testBeep(this);
		});

		$(document).on('click', '.js-device-command', function(event){
			event.preventDefault();
			app.shutdown(this, $(this).data('command'));
		});

		$(document).on('click', '.js-reset-device', function(event){
			event.preventDefault();
			app.resetDevice(this);
		});
